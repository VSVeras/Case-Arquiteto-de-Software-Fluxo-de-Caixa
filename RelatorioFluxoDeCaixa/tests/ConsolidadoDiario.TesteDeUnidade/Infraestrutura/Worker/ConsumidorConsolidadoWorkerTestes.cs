...
public sealed class ConsumidorConsolidadoWorkerTestes
{
    ...
    public async Task DeveConfirmarMensagemProcessada()
    {
        ...
        processador
            .Setup(x => x.ProcessarAsync(It.IsAny<EventoDeLancamentoCriado>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);

        var contexto = CriarContexto(processador.Object);
        var argumentos = CriarArgumentos(CriarEvento());

        await InvocarProcessamentoAsync(contexto.Worker, argumentos, CancellationToken.None);

        contexto.Canal.Verify(x => x.BasicAck(argumentos.DeliveryTag, false), Times.Once);
        contexto.Canal.Verify(x => x.BasicReject(It.IsAny<ulong>(), It.IsAny<bool>()), Times.Never);
        contexto.Canal.Verify(x => x.BasicNack(It.IsAny<ulong>(), It.IsAny<bool>(), It.IsAny<bool>()), Times.Never);
    }
    ...
    {
        var contexto = CriarContexto(Mock.Of<IProcessadorEventoLancamento>());
        var argumentos = CriarArgumentos(Encoding.UTF8.GetBytes("{ json inválido"));

        await InvocarProcessamentoAsync(contexto.Worker, argumentos, CancellationToken.None);

        contexto.Canal.Verify(x => x.BasicReject(argumentos.DeliveryTag, false), Times.Once);
    }
    ...
    {
        var contexto = CriarContexto(Mock.Of<IProcessadorEventoLancamento>());
        var argumentos = CriarArgumentos(CriarEvento(correlationId: Guid.Empty));
    ...
    public async Task DeveReenfileirarMensagemQuandoProcessamentoFalhar()
    {
        ...
        contexto.Canal.Verify(x => x.BasicNack(argumentos.DeliveryTag, false, true), Times.Once);
    }
    ...
    {
        var processador = new Mock<IProcessadorEventoLancamento>();
        var contexto = CriarContexto(processador.Object, canalAberto: false);
        var argumentos = CriarArgumentos(CriarEvento());

        await InvocarProcessamentoAsync(contexto.Worker, argumentos, CancellationToken.None);

        processador.Verify(
            x => x.ProcessarAsync(It.IsAny<EventoDeLancamentoCriado>(), It.IsAny<CancellationToken>()),
            Times.Never);
    }
    ...
    public void DeveFecharEDescartarCanalAoDescartarWorker()
    {
        ...
        contexto.Canal.Verify(x => x.Close(), Times.Once);
        contexto.Canal.Verify(x => x.Dispose(), Times.Once);
    }

    [Fact]
    public void DeveDescartarCanalFechadoSemTentarFecharNovamente()
    {
        var contexto = CriarContexto(Mock.Of<IProcessadorEventoLancamento>(), canalAberto: false);

        contexto.Worker.Dispose();

        contexto.Canal.Verify(x => x.Close(), Times.Never);
        contexto.Canal.Verify(x => x.Dispose(), Times.Once);
    }
        ...
        string? tipo)
    {
        ...
        var contexto = CriarContexto(Mock.Of<IProcessadorEventoLancamento>());
        var argumentos = CriarArgumentos(evento, correlationId, tipo);
        ...
        bool canalAberto = true)
    {
        ...
        canal.SetupGet(x => x.IsOpen).Returns(canalAberto);
        ...
        var worker = new ConsumidorConsolidadoWorker(
        ...
        typeof(ConsumidorConsolidadoWorker)
            ...
            .SetValue(worker, canal.Object);

        return new ContextoDoWorker(worker, canal);
    }
        ...
        string? tipo = null)
    {
        return CriarArgumentos(
            JsonSerializer.SerializeToUtf8Bytes(evento, new JsonSerializerOptions(JsonSerializerDefaults.Web)),
            correlationId ?? evento.CorrelationId.ToString(),
        ...
        string? tipo = null)
    {
        ...
        propriedades.SetupProperty(x => x.CorrelationId, correlationId);
        propriedades.SetupProperty(x => x.Type, tipo);
    ...
    private static EventoDeLancamentoCriado CriarEvento(Guid? correlationId = null)
    {
    ...
    {
        var metodo = typeof(ConsumidorConsolidadoWorker)
            .GetMethod("ProcessarMensagemAsync", BindingFlags.Instance | BindingFlags.NonPublic)!;

        var tarefa = (Task)metodo.Invoke(worker, [argumentos, cancellationToken])!;
        await tarefa;
    }
